...
phaesI_strat.f <- function(pcuts, dt, design.factor){
  ...
  # too little interval-censored cases (95 with a1>0), a lot of a1=0
  # if(style=="a1")
  
  if(style=="a2"){
    
    c_cut <- quantile(subset(dt,is.finite(a2) & a2>0)$a2, p=pcuts)
    cbar <- cut(subset(dt,is.finite(a2) & a2>0)$a2, c(-0.01,c_cut,Inf), labels = paste0(1:(length(c_cut)+1)))
    dt$cbar <- ifelse(is.finite(dt$a2) & dt$a2>0, cbar, 0)
    
    g <- xtabs(~cbar, dt)
    group <- factor(paste0(dt$cbar))
    group.mat <- expand.grid(0:3)
  ...
  if(style=="a2_d2"){
    ...
    g <- xtabs(~cbar+d2, dt)
    group <- factor(paste0(dt$cbar, dt$d2))
    #group.mat <- expand.grid(0:3, 0:2)
    #ref.group <- paste0(group.mat[,1], group.mat[,2])
    #strata.n <- as.data.frame(g)$Freq # give size of each strata
    dt$a2d2 <- group
    g <- xtabs(~a2d2, dt, addNA = TRUE)
    ref.group <-  as.character(as.data.frame(g)$a2d2) #c( "00", "01", "11", "21")
    strata.n <- as.data.frame(g)$Freq # give size of each strata
  ...
  if(style=="a2_d1_d2"){
    ...
    dt$d12 <- factor(paste0(dt$d1, dt$d2))
    group <- factor(paste0(dt$cbar, dt$d12))
    
    #g <- xtabs(~cbar+d12, dt, addNA = TRUE)
    g <- xtabs(~group, addNA = TRUE)
    #group.mat <- expand.grid(0:3, c("00", "01", "11", "20", "21"))
    ref.group <- as.character(as.data.frame(g)$var1)#paste0(group.mat[,1], group.mat[,2])
    strata.n <- as.data.frame(g)$Freq # give size of each strata
  ...
  #   
  #   g <- xtabs(~x1+d1, dt)
  #   group <- factor(paste0(dt$x1, dt$d1))
  #   group.mat <- expand.grid(0:1, 0:2)
  #   ref.group <- paste0(group.mat[,1], group.mat[,2])
  #   strata.n <- as.data.frame(g)$Freq # give size of each strata
  #   
  # }
  ...
  if(style=="x1_d2"){
    ...
    g <- xtabs(~x1+d2, dt)
    group <- factor(paste0(dt$x1, dt$d2))
    #group.mat <- expand.grid(0:1, 0:1)
    #ref.group <- paste0(group.mat[,1], group.mat[,2])
    #strata.n <- as.data.frame(g)$Freq # give size of each strata
    dt$x1d2 <- group
    g <- xtabs(~x1d2, dt, addNA = TRUE)
    ref.group <-  as.character(as.data.frame(g)$x1d2) #c( "00", "01", "11", "21")
    strata.n <- as.data.frame(g)$Freq # give size of each strata
  ...
  if(style=="x1_a2_d2"){
    ...
    g <- xtabs(~x1+cbar+d2, dt)
    group <- factor(paste0(dt$x1, dt$cbar, dt$d2))
    #group.mat <- expand.grid(0:1, 0:3, 0:1)
    #ref.group <- paste0(group.mat[,1], group.mat[,2], group.mat[,3])
    #strata.n <- as.data.frame(g)$Freq # give size of each strata
    dt$x1a2d2 <- group
    g <- xtabs(~x1a2d2, dt, addNA = TRUE)
    ref.group <-  as.character(as.data.frame(g)$x1a2d2) #c( "00", "01", "11", "21")
    strata.n <- as.data.frame(g)$Freq # give size of each strata
    
  }
  
  # if(style=="x1_a_d1_d2"){ # zero strata exist
