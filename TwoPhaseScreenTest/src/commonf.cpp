...
void copyDblToNumVec(double *dvec, int n, NumericVector ret) {
  for (int i=0; i<n; i++) ret[i] = dvec[i];
}

void copyIntToIntVec(int *ivec, int n, IntegerVector ret) {
  for (int i=0; i<n; i++) ret[i] = ivec[i];
}
...
extern "C" {
  ...
  void C_ppwc(double *R_q, double *R_cuts, double *R_levels, int *R_lower, int *R_logInd,
         int *R_ncuts, int *R_nlevels, double *ret) {

    NumericVector cuts(*R_ncuts);
    NumericVector levels(*R_nlevels);
    copyDblToNumVec(R_cuts,   *R_ncuts,   cuts);
    copyDblToNumVec(R_levels, *R_nlevels, levels);

    *ret = ppwc(*R_q, cuts, levels, *R_lower, *R_logInd);
